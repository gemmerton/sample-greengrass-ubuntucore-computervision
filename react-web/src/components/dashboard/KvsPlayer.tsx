...
export const KvsPlayer: React.FC<KvsPlayerProps> = ({
  ...
  const displayStatus = error ? 'error' : loading ? 'buffering' : playing ? 'streaming' : 'offline';
  const statusLabel = error ? `Error: ${error}` : loading ? 'Loading...' : playing ? 'Live' : 'Offline';

  return (
    <div className="kvs-player-container">
      <video ref={videoRef} data-testid="kvs-video" muted playsInline />
      <div className={`kvs-player-overlay status-${displayStatus}`}>
        {statusLabel}
      </div>
    </div>
  );
